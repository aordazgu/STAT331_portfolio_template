...
unformatted_summary_table |>
  ...
  mutate(
    Percent = scales::label_percent(accuracy = 0.1)(prop),
    Level = level,
    Count = n
  ) |>
...
```{r}
#| label: pe-3-map-2
# Lab 9: Revisit of Lab 3, Question 4 (Revised)
...
At the start of the course, I mainly focused on getting my code to run.
As the quarter went on, my thinking shifted toward writing code that is
clear, consistent, and modern. The revisions in my portfolio show this
change. I went back to earlier labs and updated code to match the
standards set for the class. Use tidyverse, dplyr, forcats, and purrr;
avoid nested functions; use consistent syntax and formatting; use
labeled arguments; and avoid superseded or deprecated functions.

I revised code that used older or less clear approaches and rewrote it
with tools like across(), if_any(), pivot_longer(), pivot_wider(),
label_percent(), and label_dollar(). I cleaned up pipelines so each step
has a clear purpose instead of cramming too much into one line with
nested functions. I separated out helper functions (like creating a plot
object first and then passing it into style_plot()) so the logic is
easier to follow. I also fixed places where I was mixing styles, so now
similar tasks use the same patterns across labs.

This process changed how I see my own code. Not as a one-time answer,
but as something I can change and improve. By the end of the course, I
feel like I have a more intentional coding style that is easier for
other people to read and update, while still being robust and correct.

## Extending My Thinking

I extended my thinking by carrying ideas from early labs into work
later on. A simple example is how I use the pipe '|>'. Over time, I
started building full data-wrangling pipelines, read the data, clean it,
summarize it, and then send the result straight into a plot or table. 

I also extended my thinking in how I design visualizations. Early on, I
mostly focused on getting a graph to show the right numbers. Later, I
paid more attention to how a viewer would actually read the plot. For
example, I started putting information about the y-axis in the subtitle
so people don’t have to tilt their heads to read a rotated label. I
adjusted font sizes and rotated or tilted x-axis labels to avoid
overlapping numbers or long category names, and included descriptions of the graph for understanding.

## Peer Support & Collaboration

Here is my peer review from Lab 2. I really liked this one because I
went in specific sections and quickly stated what each section did right
or how it could be improved. 

"Hey Haley, you did a great job on this weeks lab.

Named arguments (ggplot(data = ..., mapping = aes(...))) used
consistently. 

Whitespace and line breaks: operators/comma spacing is good; every + is
followed by a newline; readable layers. 

Facet use (facet_wrap(~ species)) matches the task and helps reduce
overplotting.

Warnings suppressed properly for read_csv(..., show_col_types = FALSE),
which helps meet the “no warnings/messages” submission rule.

Axis label mix-up (scatterplot).

The only issue I saw was you mapped x = weight, y = hindfoot_length, but
labeled x = “Hindfoot length (mm)” and moved Body Weight (g) into the
subtitle. The subtitle is supposed to take the y-axis label; here the y
variable is hindfoot_length.

Fix: put "Hindfoot length (mm)" in the subtitle and label x as "Body
Weight (g)"."

The weekly pair programming activities are in my opinion what played the
bigest role in how I grew as a collaborator and coder. I liked the
'coder'/'computer' structure because it forced both people to listen and
be clear. As the 'coder', I had to explain why I was choosing a certain
function or pipeline, not just what to type. As the 'computer', I
practiced listening, asking  questions, and giving gentle critics when
something didn’t quite make sense.

These sessions showed me that there are many valid ways to solve the
same problem. Often, I would have one solution in mind and my partner
would suggest a different but equally valid approach. Seeing those
differences helped me be more open to alternative methods and more
flexible in my own coding. I think that growth in communication and
collaboration is one of the main reasons my R skills improved so much
over the course.