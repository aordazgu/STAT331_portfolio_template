...
```{r}
#| label: dvs-3-2
# Lab 4: Question 8 (annotated for portfolio)

label_point <- ca_childcare |>
  summarise(
    income_75 = quantile(mhi_2018, 0.75, na.rm = TRUE),
    price_75  = quantile(mc_infant, 0.75, na.rm = TRUE)
  )
...
rescale_01 <- function(x) {
  ...
  if (length(x) < 2) {
    stop("`x` must have length at least 2.")
  }
...
#| label: pe-2-3
#| label: pe-2-3
# Lab 8: Function to style a plot
